...
<main class="flex min-h-[calc(100vh-4rem)] flex-col justify-start">
    <div class="mx-auto max-w-5xl px-4 py-4 min-w-full">
        <div class="motion-preset-slide-up-lg mx-auto max-w-5xl">
            <div class="rounded-lg overflow-hidden md:h-96 h-48 w-full">
                <img src="{{url($blogpost->header_image)}}" alt="{{$blogpost->title}}" class="min-w-full object-cover object-center hover:scale-105 transition-transform duration-200">
            </div>
            ...
            <p class="flex items-center text-gray-300 mt-4 font-bold text-sm">
                ...
                Written on {{\Carbon\Carbon::parse($blogpost->publish_date)->format('F j, Y @ H:i')}} by {{$blogpost->author}}
            </p>
