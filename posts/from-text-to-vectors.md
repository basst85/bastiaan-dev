...
faq:
  - question: "What is semantic search and how is it different from keyword search?"
    answer: "Semantic search compares the meaning of a query to your data instead of matching exact words, so it can find relevant results even if they use completely different wording than the search query."
  - question: "What is a vector embedding?"
    answer: "A vector embedding is a numerical representation of a piece of text in a multi-dimensional space, generated by a language model, where texts with similar meaning end up positioned close to each other."
  - question: "Which embedding model is used in this guide?"
    answer: "OpenAI's text-embedding-3-small, configured with dimensions set to 1000 to keep the vectors compact. The model defaults to 1536 dimensions but accepts any value down to 256, since it was trained so that shortening the output doesn't lose much of its meaning-representing quality."
  - question: "Why use SQLite instead of a dedicated vector database?"
    answer: "For small to medium datasets, SQLite is lightweight, requires no separate server, and is simple to set up. It stores the embeddings as BLOBs directly alongside the text. Without a vector extension, similarity search means scanning every row and comparing it to the query, which is fine for thousands of rows but won't scale to millions without an index."
  - question: "How do you actually compare two vectors to find similar text?"
    answer: "By calculating a similarity score between the query's embedding and every stored embedding, then sorting the results by that score. OpenAI's embeddings are normalized to length 1, so the dot product gives the same ranking as cosine similarity, it's just cheaper to compute, which is why the example uses it directly."
---
## Introduction
